...
func StatsFor(body string, queryTerms []string) DocStats {
	return StatsForTokens(Tokenize(body), queryTerms)
}

// StatsForTokens is StatsFor for an already-tokenized document — use it
// when you tokenize once and score against many queries (e.g. Corpus) so
// you don't re-tokenize the body on every call.
